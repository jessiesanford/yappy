...
/**
 * Extends functionality for Screenplay editor type
 * @inheritDoc {ResolvedIterator}
 *
 */
export class ScreenplayIterator extends ResolvedIterator {
  /**
   ...
   * @param {Number|ScreenplayIterator|ResolvedIterator|ResolvedData|ResolvedPos?} obj
  ...
  /**
   * @inheritDoc
   * @return {ScreenplayIterator}
   */
  first(opt_depth = this.depth, opt_magnet = false) {
    return /** @type {ScreenplayIterator} */super.first(opt_depth, opt_magnet);
  }
  ...
  last(opt_depth = this.depth, opt_magnet = false) {
    return /** @type {ScreenplayIterator} */super.last(opt_depth, opt_magnet);
  }
  ...
  seek(offset, opt_depth = this.depth, opt_force = false) {
    return /** @type {ScreenplayIterator} */ super.seek(offset, opt_depth, opt_force);
  }
  ...
  update(opt_pos = this.data.pointer, opt_tr = null) {
    return /** @type {ScreenplayIterator} */ super.update(opt_pos, opt_tr);
  }
   ...
   */
  clone() {
    return new ScreenplayIterator(this.tr, this.data.clone(), this.data.depth);
  }
}

/**
 * Enum class for stageplay depths
 */
export enum SPDepth {
  Page = 1,
  Element,
...
if (process.env.NODE_ENV === "development") {
  global["$ScreenplayIterator"] = ScreenplayIterator;
}